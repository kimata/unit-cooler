...
class TestControlMessageList:
    ...
    def test_duty_structure(self):
        ...
        for i, msg in enumerate(CONTROL_MESSAGE_LIST):
            duty = msg.duty
            assert hasattr(duty, "enable"), f"Entry {i} missing 'enable'"
            assert hasattr(duty, "on_sec"), f"Entry {i} missing 'on_sec'"
            assert hasattr(duty, "off_sec"), f"Entry {i} missing 'off_sec'"
