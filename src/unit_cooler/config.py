...
# RuntimeSettings のフィールドをオーバーライドする環境変数（フィールド名 → 環境変数名）
_ENV_OVERRIDES: dict[str, str] = {
    "control_host": "HEMS_CONTROL_HOST",
    "pub_port": "HEMS_PUB_PORT",
    "log_port": "HEMS_LOG_PORT",
    "status_pub_port": "HEMS_STATUS_PUB_PORT",
    "actuator_host": "HEMS_ACTUATOR_HOST",
    "server_port": "HEMS_SERVER_PORT",
...
class RuntimeSettings:
    ...
    @classmethod
    def from_args(cls, args: dict[str, Any], field_map: dict[str, str]) -> Self:
        """docopt 引数と環境変数から RuntimeSettings を生成

        Args:
            args: docopt の解析結果
            field_map: フィールド名 → docopt キー（例: {"pub_port": "-p"}）

        優先順位: 環境変数 (_ENV_OVERRIDES) > CLI 引数 > フィールドデフォルト値。
        型変換は from_dict に委譲する。
        """
        data: dict[str, Any] = {}
        for field_name, arg_key in field_map.items():
            value = args.get(arg_key)

            env_name = _ENV_OVERRIDES.get(field_name)
